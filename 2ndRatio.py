...
def read_average_file(file_path):
    with sftp.open(file_path) as f:
        ...
        for line in lines:
            parts = line.split()
            if len(parts) >= 5:
                data.append((float(parts[1]), float(parts[3]), float(parts[4])))
...
# 添加标签和标题
plt.xlabel('eik / kT')
plt.ylabel('Bond Ratio (First Neighbour Ratio)')
plt.title('Bond Ratio Relationship')

# 保存为JPG文件
output_file = 'Ratio2nd20.jpg'
